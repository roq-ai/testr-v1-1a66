...
import { AccessListPage as ShowCaseList } from 'pages/accesses';
...
function DashboardPage() {
  ...
  const pdfStat = useSWR<PaginatedInterface<PdfInterface>>(
    () => `/pdfs`,
    () => getPdfs({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Pdfs',
      result: pdfStat?.data?.totalCount,
      isLoading: pdfStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Accesses',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`accesses`} passHref legacyBehavior>
                  <Button
